...
export default function WorkOrderFormTask({ activities, setActivities }) {
  ...
  const handleAddActivity = () => {
    // Create a new activity with an incremented id
    const newActivity = { name: name, description: description, asset: "", asignee:"" }
    // Update the activities state with the new activity added
        ...
        {activities.map(activity => (
          <div key={activity.id} className='taskDiv grid grid-cols-12'>
            ...
            <div className='col-span-3'>
              <div >
                <input value={activity.name} onChange={(e) => handleInputChange(activity.id, 'name', e.target.value)} />
              </div>
              <div >
                <input value={activity.description} onChange={(e) => handleInputChange(activity.id, 'description', e.target.value)} />
              </div>
            </div>
