...
import UsersAdminCard from './usersAdminCard/usersAdminCard.jsx'

export default function UsersAdminLayout({columns, titleTxt}) {
        ...
        <div>
            <div className='md:mt-4 ml-4 md:ml-8 grid  md:grid-cols-12'>
                ...
                <div className='mt-8 md:mt-1 md:col-span-1 justify-end'>
                    <AddButton buttonTxt={"new"} route={"form"} />
                </div>
            </div>
            <div className={`grid mt-8 ml-2 md:ml-8 ${cellData !== "" ? 'md:grid-cols-3' : 'card-closed'}`}>
                ...
                {cellData !== "" && <UsersAdminCard cellData={cellData} setCellData={setCellData} />}
            </div>
        </div>
    )
}
